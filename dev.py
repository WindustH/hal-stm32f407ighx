...
# Configuration
PROJECT_NAME = "hal-stm32f407ighx"
        ...
        if os.name != "nt":
            try:
                ...
                if "0483:3748" in lsusb_output:
                    ...
                    print_status(
                        "No known programmer detected via lsusb, trying OpenOCD test...",
                        "warning",
                    )
...
def main():
    ...
    build_release_parser = subparsers.add_parser(
        "build-release", help="Build in Release mode"
    )
    build_release_parser.add_argument(
        "--build-dir", default="build", help="Build directory"
    )

    # Build-debug command
    build_debug_parser = subparsers.add_parser(
        "build-debug", help="Build in Debug mode"
    )
    build_debug_parser.add_argument(
        "--build-dir", default="build", help="Build directory"
    )
