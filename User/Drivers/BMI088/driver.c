...
void bmi088_update_pose(u16 pin) {
  ...
  if (pin == gy_it_pin || pin == ac_it_pin) {
    ...
    if (pose_update_on) {
      bmi088_pose.x += imu_data.accel[0] * dt;
      bmi088_pose.y += imu_data.accel[1] * dt;
      bmi088_pose.z += imu_data.accel[2] * dt;
      bmi088_pose.roll += imu_data.gyro[0] * dt;
      bmi088_pose.pitch += imu_data.gyro[1] * dt;
      bmi088_pose.yaw += imu_data.gyro[2] * dt;
    }
  }
}
void bmi088_stop_update_pose() { pose_update_on = false; }
void bmi088_start_update_pose() { pose_update_on = true; }
...
volatile poseT *bmi088_get_pose() { return &bmi088_pose; };