...
u8 bsp_can_fifo0_cb_add(canFifo0Cb callback) {
  ...
  for (u8 i = 0; i < CAN_FIFO0_CB_LIST_SIZE; i++) {
    if (!(can_fifo0_msg_pending_cb_list.state & (1U << i))) {
      can_fifo0_msg_pending_cb_list.callbacks[i] = callback;
      can_fifo0_msg_pending_cb_list.state |= (1U << i);
      __enable_irq();
      return i;
    }
  }
...
void bsp_can_fifo0_cb_remove(u8 idx) {
  ...
  __disable_irq();
  can_fifo0_msg_pending_cb_list.state &= ~(1U << idx);
  // 可选：清空指针（非必须，但更安全）
  can_fifo0_msg_pending_cb_list.callbacks[idx] = NULL;
  __enable_irq();
}
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan) {
  ...
  for (u8 i = 0; i < CAN_FIFO0_CB_LIST_SIZE; i++) {
    if ((state_snapshot & (1U << i)) &&
      ...
      can_fifo0_msg_pending_cb_list.callbacks[i](hcan, &header, data);
    }
  }
}